...
class AgentRunner:
    ...
    def call_model_node(self, state: AgentState) -> Dict[str, Any]:
        system_prompt = (
            ...
            "2.  **Decisión de Herramientas:** Basado en la pregunta, decide qué combinación de herramientas usar:\n\n"
            "    -   **Tipo 1: Pregunta General de Póliza:** (ej: '¿qué es un deducible?', '¿cubre prótesis?')\n"
            "        -> Usa SÓLO `hybrid_opensearch_search`.\n\n"
            
            "    -   **Tipo 2: Pregunta General de Noticias/Web:** (ej: '¿qué es una ISAPRE?', 'noticias de seguros')\n"
            "        -> Usa SÓLO `web_search`.\n\n"

            "    -   **Tipo 3: PREGUNTA HÍBRIDA (Evento + Póliza):** (ej: '¿cubre el incendio de anoche?', '¿cubre el motín?', '¿cubre los daños de la tormenta X?')\n"
            "        -> **DEBES USAR AMBAS HERRAMIENTAS.** Debes generar *dos* tool calls en tu respuesta:\n"
            "        1. Una llamada a `web_search` para encontrar noticias y confirmar el evento.\n"
            "        2. Una llamada a `hybrid_opensearch_search` para buscar las cláusulas de la póliza relacionadas (ej: 'cobertura incendio', 'exclusión motín').\n\n"
    ...
    def reformulate_for_tools_node(self, state: AgentState) -> Dict[str, Any]:
        ...
        for tool_call in last_message.tool_calls:
            ...
            if tool_name in reformulation_targets and "query" in tool_args:
                query_to_fix = tool_args["query"]
                reformulation_prompt = f"""Dada la siguiente conversación y una consulta de búsqueda (query), reescribe la consulta para que sea una pregunta independiente y completa que pueda ser entendida sin el historial de chat.
                                            Si la consulta ya es independiente, devuélvela tal cual.

                                            **Historial de Chat (últimos mensajes):**
                                            {self._format_history_for_reformulation(history)}

                                            **Consulta de Búsqueda (a arreglar):**
                                            "{query_to_fix}"
                        ...
                        debug_steps.append(
                            {
                                ...
                                "tool_name": tool_name,
                                "original_query": query_to_fix,
                                "reformulated_query": new_query,
                            }
                        )
                        state["debug_steps"] = debug_steps
                except Exception as e:
                    new_tool_calls.append(tool_call)
                    if state.get("debug"):
                        state.get("debug_steps", []).append(
                            {"step": "reformulate_query_failed", "tool_name": tool_name, "error": str(e)}
                        )
            else:
                new_tool_calls.append(tool_call)
        if not modified_queries:
            return {}
        new_ai_message = AIMessage(content=last_message.content, tool_calls=new_tool_calls, id=last_message.id)
    ...
    def call_tool_node(self, state: AgentState) -> Dict[str, Any]:
        ...
        for tool_call in tool_calls:
            ...
            if not tool:
                tool_outputs.append(
                    ToolMessage(content=f"[ERROR] Tool '{tool_name}' is not available in this request.", tool_call_id=tool_id)
                )
                if state.get("debug"):
                    debug_steps.append({"tool": tool_name, "input": tool_args, "error": "tool_not_available"})
                continue
            output = tool.invoke(tool_args)
            if state.get("debug"):
                debug_steps.append(
                    {"step": "call_tool", "tool": tool.name, "input": tool_args, "output_preview": str(output)[:500]}
                )
    ...
    def run(
        ...
        *,
        ...
        resp: Dict[str, Any] = {
            ...
            "usage": {"top_k": top_k, "enable_web_search": enable_web_search, "language": language},
        }
        if debug:
            resp["debug"] = {
