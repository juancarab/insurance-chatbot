    ...
    def _build_graph(self) -> StateGraph:
        ...
        def call_model_node(state: AgentState) -> Dict[str, Any]:
            system_prompt = (
                ...
                "\n\n"
                "## Resumen rápido de comportamiento (decisión autónoma):\n"
                "Tú decides AUTÓNOMAMENTE si debes: \n"
                "  1) RESPONDER solo con tu conocimiento general (ej. saludos, preguntas "
                "     triviales o consultas que no requieren datos externos), \n"
                "  2) LLAMAR a la herramienta 'hybrid_opensearch_search' (cuando la pregunta "
                "     requiere buscar detalles de pólizas, coberturas, exclusiones o "
                "     condiciones contractuales), o \n"
                "  3) LLAMAR a la herramienta 'web_search' (cuando la pregunta requiere "
                "     noticias recientes, información pública en la web o contexto de "
                "     eventos actuales).\n\n"
                "## Decisión y formato de llamada a herramientas:\n"
                "- Evalúa la pregunta del usuario y decide SIEMPRE la opción más simple: "
                "  si es un saludo o pregunta conversacional, responde sin herramientas.\n"
                "- Solo llama a una herramienta cuando NECESITES información externa para "
                "  responder correctamente. No llames a herramientas por defecto.\n"
                "- Si llamas a una herramienta, usa exactamente su nombre: 'hybrid_opensearch_search' "
                "  o 'web_search', y pásale un único argumento llamado 'query' con el texto "
                "  de búsqueda apropiado (por defecto, la pregunta del usuario, posiblemente "
                "  reformulada para búsqueda). Ejemplo de payload: {\"query\": \"...\"}.\n"
                "- Si la pregunta claramente necesita datos de pólizas, prioriza 'hybrid_opensearch_search'.\n"
                "- Si la pregunta claramente necesita noticias o contexto reciente, prioriza 'web_search'.\n\n"
                "## Lógica de combinación de fuentes (cuando ambas herramientas son usadas):\n"
                "- Puede que llames a AMBAS herramientas cuando la pregunta conecte una póliza con "
                "  un evento actual (ej.: '¿Mi póliza cubre daños por la tormenta de anoche en X?').\n"
                "- SI ambas herramientas devuelven información RELEVANTE, tu respuesta FINAL debe: \n"
                "  1) SINTETIZAR la información de las pólizas (lo que cubre o no la póliza) con la "
                "     información web (por ejemplo, noticias, fechas, alcance geográfico) para ofrecer "
                "     una respuesta integrada y accionable; \n"
                "  2) NO presentar las fuentes como segmentos separados; en su lugar integra los hallazgos "
                "     en un único texto explicativo que muestre la relación entre la póliza y el evento; \n"
                "  3) Si una herramienta NO devuelve resultados útiles, deja claro cuál aportó la evidencia y "
                "     cuál no (por ejemplo: 'Encontré esta cláusula en la póliza, pero no hallé noticias que "
                "     confirmen un evento reciente en su zona').\n\n"
                "## Reglas de seguridad y transparencia:\n"
                "- Si tras usar las herramientas la información sigue siendo insuficiente, dilo claramente: "
                "  'No he podido encontrar información suficiente...'.\n"
                "- AL FINAL, incluye un breve bloque de fuentes con título y URL (si hay) de cada documento "
                "  o noticia que usaste, y una breve nota de cuál información provino de la póliza y cuál de la web.\n"
                "- Mantén el tono profesional y evita dar asesoría legal vinculante; sugiere consultar al proveedor o "
                "  a un asesor cuando corresponda."
            )
