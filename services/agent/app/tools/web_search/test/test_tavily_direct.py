...
def test_tavily_search_returns_results(client, query):
    ...
    assert "results" in resp, "The response must include the key: results"
    results = resp.get("results", [])

    if not results:
        pytest.xfail(f"No results for query: {query!r}. (network/recency/ratelimit)")

    assert len(results) >= 1, "Must have at least one result"
    for r in results:
